...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (file_exists($targetFile)) {
        echo "<div style='text-align: center; color: red;'>Le fichier existe déjà.</div>";
        $uploadOk = 0;
    }
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $targetFile)) {
            echo "<div style='text-align: center; color: green;'>Le fichier a été téléchargé.</div>";
        } else {
    ...
    if ($updateStmt->execute()) {
        echo "<div style='text-align: center; color: green;'>Annonce mise à jour avec succès.</div>";
    } else {
...
<body>
    ...
    <a href="gestion_annonces.php" class="btn btn-secondary" style="
